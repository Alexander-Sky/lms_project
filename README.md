...
|---|---|---|---|---|
| push в любую ветку | ✅ | ✅ | сборка + публикация | ✅ |
| pull request в `main` | ✅ | ✅ | только сборка | — |
| ручной запуск из `main` | ✅ | ✅ | сборка + публикация | ✅ |

Каждый push, прошедший тесты, сразу выкатывается на сервер — это непрерывный деплой. На pull request деплоя нет: тот же коммит уже выкатился при push.

Сервер один и разработчик один, поэтому деплоится любая ветка. В команде деплой ограничили бы веткой `main`, чтобы код из чужой непроверенной ветки не попадал на боевой сервер.

Ручной запуск: **Actions** → **CI/CD** → **Run workflow**. Кнопка появляется, только когда `ci.yml` уже есть в ветке `main` — так устроен GitHub.
...
**4. Первый деплой.** Сделайте push в любую ветку — или, если секреты заводили уже после push, перезапустите последний запуск: **Actions** → запуск → **Re-run all jobs**. Через пару минут приложение откроется на `http://<IP>/api/docs/`.

...
|---|---|---|---|---|
| push в любую ветку | ✅ | ✅ | сборка без публикации | — |
| pull request в `main` | ✅ | ✅ | сборка без публикации | — |
| push в `main` | ✅ | ✅ | сборка + публикация | ✅ |
| ручной запуск | ✅ | ✅ | сборка + публикация | ✅ |

Ручной запуск: вкладка **Actions** → **CI/CD** → **Run workflow** → выбрать ветку.
...
**2. Сервер.** При создании VPS добавьте публичный личный ключ (`~/.ssh/id_ed25519.pub`). Зайдите на сервер и запустите скрипт настройки, передав ему публичный ключ деплоя:
...
Скрипт отказывается работать, если у `root` нет SSH-ключа: иначе после отключения паролей на сервер было бы не зайти. Повторный запуск безопасен.
...
**4. Первый деплой.** **Actions** → **CI/CD** → **Run workflow**. Через пару минут приложение откроется на `http://<IP>/api/docs/`.
